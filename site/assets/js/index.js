...
function prettyLabel(raw){
  ...
  return k.split(' ').map(tok=>{
    if (/^(2d|3d|4d|rgb|rgbd|rgb-d|slam|lidar|cnn|rnn|gan|svm|ml|ai|nlp|uav|imu|sar|bim|ifc|gpr|mep|vlm|llm|qa|hvac|lod3|ui|pcd|fob|cif|dap)$/i.test(tok)) return tok.toUpperCase();
...
function formatDatasetTitle(raw){
  ...
  const acronymPattern = /^(2d|3d|4d|rgb|rgbd|rgb-d|slam|lidar|cnn|rnn|gan|svm|ml|ai|nlp|uav|imu|sar|bim|ifc|gpr|mep|vlm|llm|qa|hvac|lod3|ui|pcd|fob|cif|dap)$/i;
