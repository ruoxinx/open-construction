      ...
      ${publications.map(pub => {
        ...
        return `
          <${tag} class="publication-row${pub.url ? ' is-link' : ''}"${href} role="listitem">
            <span class="publication-title">${escapeHtml(pub.title)}${year}</span>
          </${tag}>
        `;
      }).join('')}
    </div>
...
function canonicalModalityList(val){
  if (!normalizeList(val).length) return [];
  if (window.OCTerms?.canonicalizeModalityLabels) {
    return window.OCTerms.canonicalizeModalityLabels(val);
  }
...
function prettyTermLabel(raw){
  ...
  const acronymPattern = /^(2d|3d|4d|rgb|rgbd|rgb-d|slam|lidar|cnn|rnn|gan|svm|ml|ai|nlp|uav|imu|sar|bim|ifc|idm|mvd|gpr|mep|teaser|vlm|llm|qa|hvac|lod3|ui|pcd|fob|cif|dap)$/i;
...
function canonicalAltmetricCitationHref(href){
  try {
...
let altmetricCitationLinkGuardInstalled = false;
function installAltmetricCitationLinkGuard(){
  if (altmetricCitationLinkGuardInstalled || !document?.addEventListener) return;
  altmetricCitationLinkGuardInstalled = true;
  document.addEventListener('click', event => {
    const link = event.target?.closest?.('.altmetric-embed a[href]');
    const canonical = link ? canonicalAltmetricCitationHref(link.href) : '';
    if (!link || !canonical) return;
    link.href = canonical;
  }, true);
}

function normalizeAltmetricCitationLinks(root){
  const rewrite = () => root?.querySelectorAll('.altmetric-embed a[href]').forEach(link => {
    const canonical = canonicalAltmetricCitationHref(link.href);
...
async function initializePublicationBadges(root){
  ...
  if (hasAltmetric) {
    installAltmetricCitationLinkGuard();
    normalizeAltmetricCitationLinks(root);
  }
  const loads = [];
...
function citationExportSourceHtml(doiVal, records){
...
function citationExportRequest(doi, format){
  ...
  const fallbackUrl = `https://doi.org/${encodedDoi}`;
  if (format === 'bibtex') {
    return {
      url,
      accept: 'application/x-bibtex',
      fallbackRequests: [{ url: fallbackUrl, accept: 'application/x-bibtex' }],
      extension: 'bib',
  ...
  if (format === 'ris') {
    return {
      ...
      fallbackRequests: [{ url: fallbackUrl, accept: 'application/x-research-info-systems' }],
      extension: 'ris',
  ...
  return {
    ...
    fallbackRequests: [
      { url: `https://citation.doi.org/format?doi=${encodedDoi}&style=${encodeURIComponent(style)}&lang=en-US`, accept: 'text/plain' },
      { url: fallbackUrl, accept: `text/x-bibliography; style=${encodeURIComponent(style)}; locale=en-US` }
    ],
    extension: 'txt',
  ...
  const getCitation = async format => {
    if (!citationCache.has(format)) {
      ...
      citationCache.set(format, request ? (async () => {
        ...
        for (const fallback of request.fallbackRequests || []) {
          const value = await fetchScholarlyText(fallback.url, fallback.accept);
...
function renderCitationExportSource(root, records, doi){
  ...
  if (!host) return;
  const source = citationExportSourceHtml(doi, records);
  if (!source) {
    host.hidden = true;
    host.innerHTML = '';
    return;
  ...
  try{
    if (type === 'model') {
      ...
      function relatedCatalogHref(targetType){
        return `../related-resources.html?source_type=model&source_id=${encodeURIComponent(m.id || m.title || '')}&target_type=${encodeURIComponent(targetType)}`;
      }
    ...
    const relatedDatasets = datasets
          ...
          <div class="col-lg-6">
            <div class="detail-subcard h-100">
              <div class="detail-subhead-row">
                ...
                <span class="related-section-source"><span>Data source:</span><a class="related-section-source-link" href="${relatedCatalogHref('model')}" aria-label="View related models in OpenConstruction">${openConstructionIconHtml()}<span>OpenConstruction</span></a></span>
              </div>
              ${relatedModels.length ? relatedModels.map(other => `
                <a class="related-link" href="${modelHref(other)}">
                  ...
                  <span class="related-link-meta">${escapeHtml(truncateText([uniquePrettyTerms(other.tasks || other.task)[0], canonicalModalityList(other.modalities || other.modality || other.data_modalities)[0], uniquePrettyTerms(other.applications || other.application)[0]].filter(Boolean).join(' • ') || 'Similar task or application area', 90))}</span>
                </a>
              `).join('') : '<p class="text-muted small mb-0">No related models were identified from the current catalog.</p>'}
              ${relatedModels.length ? `<a class="related-section-more" href="${relatedCatalogHref('model')}">View all related models <span aria-hidden="true">&rarr;</span></a>` : ''}
            </div>
          </div>
          <div class="col-lg-6">
            <div class="detail-subcard h-100">
              <div class="detail-subhead-row">
                ...
                <span class="related-section-source"><span>Data source:</span><a class="related-section-source-link" href="${relatedCatalogHref('dataset')}" aria-label="View related datasets in OpenConstruction">${openConstructionIconHtml()}<span>OpenConstruction</span></a></span>
              </div>
              ${relatedDatasets.length ? relatedDatasets.map(ds => `
                <a class="related-link" href="${datasetHref(ds)}">
                  ...
                  <span class="related-link-meta">${escapeHtml(truncateText([uniquePrettyTerms(ds.potential_tasks)[0], canonicalModalityList(ds.data_modality)[0]].filter(Boolean).join(' • ') || 'Relevant training or evaluation dataset', 90))}</span>
                </a>
      ...
      const mainHero = `
        <style>
          ...
          .detail-primary-actions{ display:inline-flex; align-items:center; gap:.15rem; margin-left:.25rem; }
          .oc-follow-inline,.scholarly-cite-inline{ display:inline-flex; align-items:center; justify-content:center; height:30px; min-height:30px; gap:.25rem; margin:0; padding:0; border:0; border-radius:0; background:transparent; color:var(--oc-text); font-size:.8rem; font-weight:500; line-height:1; text-decoration:none!important; vertical-align:middle; }
          ...
          .publication-list{ display:grid; gap:.45rem; margin-top:.45rem; }
          .publication-row{ display:block; padding:.18rem 0; color:inherit; text-decoration:none; }
...
.oc-abs-toggle:hover{ text-decoration:underline; }
    ...
    const datasetModalityList = canonicalModalityList(ds.data_modality);
    ...
    function relatedCatalogHref(targetType){
      return `../related-resources.html?source_type=dataset&source_id=${encodeURIComponent(ds.id || ds.name || '')}&target_type=${encodeURIComponent(targetType)}`;
    }
    function scoreOverlap(a, b){
      const setA = new Set(normalizeList(a).map(normKey));
      const setB = new Set(normalizeList(b).map(normKey));
    ...
    const relatedDatasets = Object.values(dataObj || {})
      ...
      .map(other => {
        ...
        const modalityScore = scoreOverlap(datasetModalityList, canonicalModalityList(other.data_modality));
    ...
    const relatedModels = modelArr
      .map(model => {
        ...
        const modalityScore = scoreOverlap(datasetModalityList, canonicalModalityList(model.modalities || model.modality || model.data_modalities));
          ...
          <div class="detail-subcard h-100">
            ...
            ${relatedModels.length ? relatedModels.map(model => `
              <a class="related-link" href="${modelHref(model)}">
                ...
                <span class="related-link-meta">${escapeHtml(truncateText([uniquePrettyTerms(model.tasks || model.task)[0], canonicalModalityList(model.modalities || model.modality || model.data_modalities)[0], uniquePrettyTerms(model.applications || model.application)[0]].filter(Boolean).join(' • ') || 'Likely compatible with this dataset', 90))}</span>
              </a>
            `).join('') : '<p class="text-muted small mb-0">No related models were identified from the current catalog.</p>'}
            ${relatedModels.length ? `<a class="related-section-more" href="${relatedCatalogHref('model')}">View all related models <span aria-hidden="true">&rarr;</span></a>` : ''}
          </div>
          ...
          <div class="detail-subcard h-100">
            ...
            ${relatedDatasets.length ? relatedDatasets.map(({ other, sharedTasks }) => `
              <a class="related-link" href="${datasetHref(other)}">
                ...
                <span class="related-link-meta">${escapeHtml(truncateText([uniquePrettyTerms(other.potential_tasks || other.tasks || other.task)[0], canonicalModalityList(other.data_modality)[0]].filter(Boolean).join(' • ') || 'Similar task coverage', 90))}</span>
              </a>
            `).join('') : '<p class="text-muted small mb-0">No related datasets were identified from the current catalog.</p>'}
            ${relatedDatasets.length ? `<a class="related-section-more" href="${relatedCatalogHref('dataset')}">View all related datasets <span aria-hidden="true">&rarr;</span></a>` : ''}
          </div>
    ...
    const mainHero = `
      ...
      <section id="overview" class="detail-section">
        ...
        <dl class="meta mb-0">
          ...
          ${metaRow('Modalities', chipLane(datasetModalityList))}
