// OpenConstruction · OER page logic (dataset-style, with Added date line)
(function () {
  ...
  function render(list){
    ...
    list.forEach(r=>{
      ...
      const added = r.added ? new Date(r.added).toISOString().split('T')[0] : '';
          ...
          <article class="resource-card h-100">
            ...
            <div class="card-body d-flex flex-column justify-content-between h-100">
              ...
              <div class="d-flex justify-content-between align-items-center mt-2 pt-2 border-top">
                ...
                <span class="small text-muted">Added ${added}</span>
              </div>
            </div>
          </article>
        </div>
  ...
  function applyFilters(){
    ...
    if(q){
      list = list.filter(r => has(r.title,q) || has(r.provider,q) || tokens(r.topics).some(t=>has(t,q)));
    }
    if(langSel.length){
